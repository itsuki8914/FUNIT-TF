...
class BatchGenerator:
    def __init__(self, img_size, imgdir, num_classes, aug=True):
        ...
        #self.direction = np.arange(num_classes)
...
if __name__ == '__main__':
    ...
    img_size = 128
    bs = 16
    btGen = BatchGenerator(img_size=img_size, imgdir=TRAIN_DIR, num_classes=num_classes)

    # sample images
    _Z = np.zeros([num_classes,img_size,img_size,3])

    _X, x_atr, _, _,  = btGen.getBatch(bs)
    _Z = (_X + 1)*127.5
    print(x_atr)
    _Z = tileImage(_Z)
